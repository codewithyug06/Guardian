        ...
        with input_c2:
            if st.button("SEND", type="primary", use_container_width=True) and q:
                ...
                try:
                    ...
                    if tool_llm and "final_state" in st.session_state:
                        context = str(st.session_state.final_state)
                        resp = tool_llm.invoke(f"System Context: {context[:2000]}\nUser: {q}\nAnswer as Guardian AI.").content
                    else:
                        resp = "I am Guardian AI. I am currently offline or lack telemetry data."
                except Exception as e:
                    resp = "Communication Error with Core Intelligence."
