...
from .tools import (
    ...
    transcribe_audio_simulation, draft_policy_update,
    simulate_digital_twin, generate_decision_hash
)
from .data_pipeline import scan_vendor_supply_chain
...
class CoderOutput(BaseModel):
    generated_code: str = Field(description="The generated python code snippet. Must be valid executable python code.")
...
def scout_agent(state: AgentState):
    ...
    codebase = state.get("user_codebase_context")
    if codebase:
        # Custom codebase analysis
        if retries == 0:
            return {
                "findings": ["Scout (Attempt 1): Scanning uploaded codebase for vulnerabilities..."],
                "scout_confidence": "High",
                "scout_retries": retries + 1
            }
        
        # Simulate local codebase scan
        violations = []
        if "credit_card" in codebase.lower() or "password" in codebase.lower():
            violations.append("Found hardcoded sensitive keys/card references in source code.")
        
        res = "Codebase Scan Complete. " + (" ".join(violations) if violations else "No severe violations found in code.")
        confidence = "High"
    else:
    ...
    vendor_scan = scan_vendor_supply_chain()
...
def coder_agent(state: AgentState):
    ...
    if risk not in ["HIGH", "CRITICAL"]: return {"generated_code": "# System Nominal", "coder_retries": retries}
    
    try:
        if model:
            structured_model = model.with_structured_output(CoderOutput)
            res = structured_model.invoke(f"Write a Python function to implement this fix. Return ONLY the code. Plan: {plan}")
            code = res.generated_code
        else:
            raise Exception("No model")
    except Exception as e:
        print(f"Coder fallback due to {e}")
        code = """
...
"""
    return {"generated_code": code.strip(), "coder_retries": retries + 1}
